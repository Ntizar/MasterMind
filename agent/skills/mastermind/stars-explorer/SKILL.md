...
- **Avalancha de catch-up tras días apagado (2026-09-15, fix real).** Tras 6 días con el PC apagado, el arranque disparó los 13 jobs vencidos en el mismo tick → límite de NaN (5 peticiones paralelas) saturado → 9 jobs LLM con `HTTP 429 max_parallel_requests` (los `no_agent` pasaron). Fix: `hermes config set cron.max_parallel_jobs 1` (default = unbounded). Documentado con detalle en `mastermind-system-ops/references/cron-catchup-429.md`.
