...
- **Indexador no ve cambios de contenido (verificado 2026-09-01):** `scripts/indexar-skills.py` solo indexa IDs nuevos (`todo = [d if i not in existing]`) — un skill **parcheado** (mejora/upgrade) queda obsoleto en ChromaDB. Al hacer `category: "upgrade"`, re-indexar el skill concreto con un script temporal que importe `indexar-skills.py` vía `importlib.util.spec_from_file_location` (el guion medio impide import normal), ejecute `mod.skill_docs()`, filtre el id `skill:<ruta/rel>` y haga `col.upsert(ids, documents, embeddings=mod.embed([text]), metadatas)`. Usar el Python del sistema (chromadb), no el venv Hermes.
...
- **execute_code bloqueado en cron:** en modo cron programático solo hay `terminal`; para lógica Python de edición de registry escribir un script temporal en `%LOCALAPPDATA%\Temp` y ejecutarlo con `python`.
