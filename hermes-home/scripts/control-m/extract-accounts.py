...
Uso:
    python3 extract-accounts.py /path/to/accounts.xlsx [--output output.json]

Salida:
    JSON con estructura de cuentas filtradas por país y tier prospectable.
"""
import zipfile
...
def extract_strings(xf):
    """Extraer sharedStrings del XLSX."""
    try:
...
def main():
    ...
    seen_names = set()
    for row in rows[1:]:
        vals = row + [''] * (len(headers) - len(row))  # Pad short rows
