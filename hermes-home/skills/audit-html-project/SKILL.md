...
- `references/single-file-interactive-audit.md` — Procedimiento para auditar aplicaciones web monolíticas (1 archivo HTML+CSS+JS inline): visores interactivos, Leaflet, etc. Diferente del enfoque multi-file educativo. Incluye checklist de errores: variables no declaradas, comillas rotas en onclick, lecturas duplicadas, estado global no limpio, UX (Escape/debounce).
