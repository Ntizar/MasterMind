...
### ⚠️ Nota sobre --nz-gradient-aurora (2026-06-22)

**FIX APLICADO:** La skin `aurora` ahora redefine `--nz-gradient-aurora` para usar solo azul → naranja (sin violeta). Commit `3d2a7ba` en Ntizar-Aurora.

**Si se ve morado/violeta en un proyecto Aurora:**
1. Verificar que se carga `ntizar.themes.css` (donde está el fix)
2. Verificar que el body tiene `data-nz-skin="aurora"`
3. Si se usa `data-nz-color-system="oklch"`, el gradiente aurora OKLCH sigue usando `hue+50°` = violeta. Redefinir manualmente si es necesario.

**Skin aurora actual:** `linear-gradient(135deg, #2563eb 0%, #f97316 100%)` — solo azul y naranja.
