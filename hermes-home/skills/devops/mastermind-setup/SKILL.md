...
- **2026-06-30:** **Nuevo patrón sin rsync: `cp -rf` con glob `*`.** `cp -rf /hermes-home/skills/* /dest/skills/` copia el CONTENIDO del directorio sin nesting (el glob expande los archivos, no la carpeta). Más corto que `rm -rf` + `cp -a`. Confirmado funcionando con 1157 skills y 2419 archivos. Ver references/backup-rsync-fallback.md.
