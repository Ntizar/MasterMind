...
## Patrones de Cron Avanzado

### Cron Batch Orchestration (absorbido de `cron-batch-orchestration`)
...
**Flujo:** Fork → Planificar secuencia → Crear crons escalonados → Verificar
**Reglas:** Cada cron autocontenido, horarios escalonados (cada hora), último cron = auditoría, repo base NUNCA se modifica.

### Cron Session Execution (absorbido de `cron-session-execution`)
Ejecutar sesiones de proyecto siguiendo un MEGA-PLAN.md. Cada sesión = un cron one-shot autocontenido.

**Flujo:** Leer MEGA-PLAN.md → Ejecutar script → Verificar outputs → Actualizar plan → Commit + Push
**Reglas:** Si una sesión falla → NO avanzar, MEGA-PLAN.md es fuente de verdad, un cron = una sesión.

### Pitfalls de crons
- **Crons paralelos NO modifican mismos archivos** — staggerear 30+ min o consolidar en un solo cron
- **Script placeholder ≠ implementación real** — verificar output directamente
- **MEGA-PLAN.md puede ya tener sesión marcada** — verificar antes de sobrescribir
- **Re-running corrompe estado** — restaurar desde último commit limpio con `git show <commit>:<archivo>`
- **Token truncado en logs** — leer GITHUB_TOKEN desde `/hermes-home/.env`
