...
### 7. CDN MIME Type Verification

**WHEN frontend loads third-party libraries via CDN and they don't work:**

Check the actual MIME type served by the CDN — this is the #1 silent failure in vanilla JS projects:

```bash
curl -sI "https://cdn.jsdelivr.net/npm/docx@9.7.1/dist/index.umd.cjs" | grep -i content-type
# application/node → BLOCKED by browser

curl -sI "https://unpkg.com/docx@9.7.1/dist/index.umd.cjs" | grep -i content-type
# text/javascript → OK
```

**Common pitfall:** jsdelivr serves `.cjs` files as `application/node` which modern browsers reject silently. Switch to unpkg which serves `text/javascript`. The symptom: network shows HTTP 200, but `window.libName` is `undefined` with NO console error.

**Check list:**
- [ ] Verify CDN URL returns the expected file (not a 404/redirect)
- [ ] Verify Content-Type is `text/javascript` or `application/javascript`
- [ ] Add defensive check: `if (typeof window.libName === 'undefined') { alert('No cargada'); return; }`
- [ ] Check that SPA fallback (serving index.html for everything) isn't intercepting CDN paths

### 8. DOM Element Existence Against JS References

**WHEN UI elements were recently removed from HTML:**

Check that ALL JavaScript references to removed DOM elements are also removed:

```bash
# Find all JS references to a removed element
grep -rn "shpModosContainer" js/
grep -rn "btnDescargarSHP" js/
grep -rn "getElementById.*shp" js/
```

**Pattern:** Remove `<div id="shpModosContainer">` from HTML but JS still references `document.getElementById('shpModosContainer')` or adds event listeners to it → JS silently fails.

**Fix:**
1. Remove the dead JS code entirely (preferred)
2. Guard with `if (document.getElementById('elID')) { ... }` as fallback
...
- [ ] CDN MIME types verified (if third-party libraries used)
- [ ] DOM references match HTML elements
...
## Phase 1.5: Multi-Version Project Detection

**Activar cuando:** El workspace contiene MÁS DE UN directorio del mismo proyecto (ej: `TimeIneco` + `TimeIneco2`, `nogal9` + `nogal9-web`).

Este es un patrón común en el workspace de David — proyectos con versiones paralelas donde una es la activa y la otra es legacy/archivo.

### 1. Detectar versiones duplicadas

```bash
# Buscar directorios del proyecto
find /root/workspace -maxdepth 2 -type d -iname "*proyecto*" 2>/dev/null
```

### 2. Verificar qué servidor está corriendo (si aplica)

```bash
# Buscar procesos Node
ps aux | grep -i node | grep -v grep | grep -v tsserver | grep -v lsp

# Buscar puertos abiertos
ss -tlnp 2>/dev/null || netstat -tlnp 2>/dev/null
```

### 3. Comparar feature parity entre versiones

Señales de que una versión es **incompleta** (la que NO deberías usar):
- `server.mjs` mucho más pequeño que la otra versión (ej: 354 vs 935 líneas)
- Falta `.env` con API keys
- Falta citybikes.js, nap.js, o módulos clave
- Los JS del frontend son v0.x vs v0.y del otro
- `data/` tiene menos archivos

### 4. Determinar la versión activa

Factores a favor de una versión:
- Tiene `.env` con credenciales
- Tiene `package.json` que se puede arrancar con `npm start`
- Los JS son más recientes (fecha de modificación más cercana)
- El `main.js` importa más módulos → más features
- El `server.mjs` tiene más líneas → más endpoints

### 5. Preguntar al usuario antes de asumir

Si no está claro qué versión es la activa, **preguntar**. No asumir. El usuario sabe cuál está usando.

### Señales de que la versión SIMPLIFICADA es sospechosa

- **server.mjs < 400 líneas** para un proyecto que requiere proxys a múltiples APIs externas (ORS, Nominatim, NAP, CityBikes, GTFS) → la versión completa suele tener 800-1200 líneas
- **No tiene .env** → no tiene claves API configuradas → llamadas a APIs externas fallarán
- **No citybikes.js** → el panel de bici-sharing no cargará
- **Código marcado como v0.8 vs v0.9** del otro → versión anterior

### Caso real: TimeIneco (Jun 2026)

| Indicador | TimeIneco (original) ✅ | TimeIneco2 ❌ |
|-----------|------------------------|---------------|
| server.mjs | 935 líneas | 354 líneas |
| .env con keys | ✅ Sí | ❌ No |
| citybikes.js | ✅ Sí | ❌ No |
| main.js | v0.9 (531 líneas) | v0.8 (422 líneas) |
| Última modif | 12:30-12:42 | 09:01-09:13 |

**Conclusión:** La versión original era la activa. TimeIneco2 era un intento de rewrite incompleto.
...
3. Auditar todo el proyecto ANTES de declarar victoria:
   ...
   - CDN MIME types (curl -sI para cada script externo)
   - DOM references (getElementById que ya no existen)
...
delegate_task(
    ...
    context="""
    Check for: syntax errors, brace balance, CDN MIME types (curl each CDN script URL),
    DOM elements referenced in JS that no longer exist in HTML,
    API mismatches (frontend calls vs backend routes),
...
- `references/frontend-dashboard-patterns-cdn-abortcontroller-ux.md` — CDN MIME type, AbortController timeout, UI progresivo (caso TimeIneco 2026) — referenciado desde el skill `frontend-dashboard-patterns`