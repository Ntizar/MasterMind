...
Uso:
    python3 generate-report.py <índice> [índice2 ...]     # Por índice (1-based)
    python3 generate-report.py --name "Nombre Empresa"     # Por nombre
    python3 generate-report.py --tier A                    # Todas Tier A
    python3 generate-report.py --segment "Banking"         # Por subsegmento
    python3 generate-report.py --limit 10                  # Primeras N cuentas
    python3 generate-report.py --all --pdf                 # Todas + PDF
...
def generate_report(acct, output_dir):
    ...
    # Datos de tamaño
    size = acct.get('size', {})
    employees = f"{size.get('empleados', 'N/A')} empleados" if size.get('empleados') else 'N/A'
    revenue = size.get('facturacion', 'N/A')
    offices = f"{size.get('oficinas', 'N/A')} oficinas" if size.get('oficinas') else 'N/A'
    hq = size.get('sede', 'N/A')
...
"""
    ...
    try:
        ...
        print(f"⚠ Error convirtiendo a PDF: {e}", file=sys.stderr)
        os.remove(js_file)
        return None
    except subprocess.TimeoutExpired:
        print(f"⚠ Timeout convirtiendo a PDF", file=sys.stderr)
        os.remove(js_file)
        return None
...
def load_accounts():
    ...
    if not os.path.exists(ACCOUNTS_FILE):
        ...
        print("   Ejecuta primero: python3 extract-accounts.py <archivo.xlsx>")
        sys.exit(1)
...
def main():
    ...
    while i < len(args):
        ...
        if arg == '--name' and i + 1 < len(args):
            ...
            indices.append(int(arg) - 1)  # 1-based to 0-based
            i += 1
        else:
            i += 1
    ...
    if all_flag:
        ...
        selected = accounts[:10]  # Default: primeras 10
    ...
    for i, acct in enumerate(selected):
        try:
            ...
            print(f"[{i+1}/{len(selected)}] {acct['account'][:50]:50s} → {basename}")
            generated += 1
