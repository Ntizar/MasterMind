...
cd scripts && NAN_API="$NAN_API" python3 indexar-skills.py 2>&1 | tail -5

# 2. Ebbinghaus decay
echo ""
echo "[2/7] 🧠 Ejecutando Ebbinghaus decay..."
python3 scripts/ebbinghaus-decay.py 2>&1 | tail -5

# 3. Knowledge graph
echo ""
echo "[3/7] 🕸️  Actualizando knowledge graph..."
python3 scripts/knowledge-graph.py 2>&1 | tail -5

# 4. Skill lifecycle
echo ""
echo "[4/7] 📊 Actualizando skill lifecycle..."
python3 scripts/skill-lifecycle.py 2>&1 | tail -5

# 5. Generate dashboard
echo ""
echo "[5/7] 📈 Actualizando dashboard..."
python3 scripts/generate-dashboard.py 2>&1 | tail -5
...
if [ "$COLLECTIONS" = "0" ]; then
    ...
    bash scripts/start-chromadb.sh 2>&1
    sleep 3
    COLLECTIONS=$(curl -s --max-time 10 http://localhost:8000/api/v1/collections 2>/dev/null | python3 -c "import sys,json; d=json.load(sys.stdin); print(len(d))" 2>/dev/null || echo "0")
